#!/usr/bin/env node
// Development setup script for Kids Tasks Card
...
console.log('🛠️  Kids Tasks Card - Development Setup');

// Read configuration
const configPath = path.join(projectRoot, 'dev-config.json');
let config = {};
...
try {
  ...
  console.warn('⚠️  Could not load dev-config.json, using defaults');
}
...
// Create development HTML file for testing
const devHtmlPath = path.join(distPath, 'dev.html');
const devHtml = `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <script>
    ...
    console.log('🛠️ Kids Tasks Card Development Environment');
    console.log('📦 Mock HASS object available as window.mockHass');
...
fs.writeFileSync(devHtmlPath, devHtml.trim());
console.log('📄 Created development HTML file');
...
console.log('3. Open: http://localhost:8080/dist/dev.html');
console.log('');
console.log('For Home Assistant integration:');
console.log('- Update your HA configuration to point to the development build');
console.log('- Use: npm run deploy:local to copy builds to HA');